...
/**
 * A trailing `*` on the id portion (before any `@row,col`, e.g. `A*` or
 * `A*@0,0`) marks the tile orientation-locked (issue #414 -- see
 * `tile-model.ts`'s `Tile.locked` / `compound-tile-model.ts`'s
 * `CompoundTile.locked` doc comments). Stripped here so it never becomes
 * part of the actual id; only needs to appear once per tile (its anchor
 * line is enough -- continuation lines never carry an id field at all, see
 * below).
 */
function parseIdAndOffset(idField: string, lineNumber: number): { baseId: string; offset: CellOffset; locked: boolean } {
  const at = idField.indexOf("@");
  const idPart = at === -1 ? idField : idField.slice(0, at);
  const locked = idPart.endsWith("*");
  const baseId = locked ? idPart.slice(0, -1) : idPart;
  if (at === -1) {
    if (baseId === "") throw new Error(`Line ${lineNumber}: tile id can't be empty (just "*")`);
    return { baseId, offset: { row: 0, col: 0 }, locked };
  }
  ...
  return { baseId, offset: { row: Number(match[1]), col: Number(match[2]) }, locked };
}
...
export function parseCompoundTileSetText(text: string): CompoundTileSet {
  ...
  const lockedByBaseId = new Map<string, boolean>();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (idField === CONTINUATION_MARKER) {
      ...
      let locked: boolean;
      ({ baseId, offset, locked } = parseIdAndOffset(idField, i + 1));
      if (locked) lockedByBaseId.set(baseId, true);
    }
  ...
  for (const [baseId, raw] of rawByBaseId) {
    ...
    const tile = buildCompoundTile(baseId, cellsInput, lockedByBaseId.get(baseId));
...
export function compoundTileSetToText(tileSet: CompoundTileSet): string {
  ...
  for (const tile of tileSet.tiles) {
    ...
    const displayId = tile.locked ? `${tile.id}*` : tile.id;
    for (const offset of tile.footprint) {
      const cell = tile.cells.get(offsetKey(offset))!;
      const idField = isUnit ? displayId : `${displayId}@${offset.row},${offset.col}`;
