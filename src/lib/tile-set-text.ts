...
export function parseTileSetText(text: string): TileSet {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const [id, n, e, s, w] = parts as [string, string, string, string, string];
    if (seenIds.has(id)) throw new Error(`Line ${i + 1}: duplicate tile id "${id}"`);
    seenIds.add(id);
    tiles.push({ id, edges: { N: n, E: e, S: s, W: w } });
  }
...
export function tileSetToText(tileSet: TileSet): string {
  return tileSet.tiles.map((t) => [t.id, ...DIRECTIONS.map((d) => t.edges[d])].join(" ")).join("\n");
}
